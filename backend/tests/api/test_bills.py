...
def test_list_bills_exposes_originating_house(test_client) -> None:
    client, app, mock_db = test_client
    _setup_list_mock(mock_db, [_SAMPLE_ROW])
    response = client.get("/api/v1/bills")
    assert response.json()[0]["originating_house"] == "Lords"
...
def test_get_bill_returns_detail(test_client) -> None:
    ...
    assert response.json()["originating_house"] == "Lords"


def test_get_bill_stages_returns_list_with_divisions(test_client) -> None:
    client, app, mock_db = test_client
    (
        mock_db.table.return_value
        .select.return_value
        ...
        {
            "id": 56,
            "stage_name": "Committee Stage",
            "house": "Commons",
            "sort_order": 3,
            "last_update": "2024-11-13T10:00:00",
            "bill_stage_sittings": [],
            "parliamentary_divisions": [],
        },
    ]
    ...
    # Agreed without a division — the stage is still present, just with no divisions.
    assert data[1]["stage_name"] == "Committee Stage"
    assert data[1]["divisions"] == []
