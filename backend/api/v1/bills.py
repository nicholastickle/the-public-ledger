...
class StageDetail(BaseModel):
    ...
    sittings: list[str]
    # Empty ⇒ the stage was agreed without a division ("on the nod").
...
async def get_bill_stages(bill_id: int, request: Request) -> list[dict]:
    """Every stage the bill has passed through (or is at), most-recently-added
    last, each carrying the division(s) — if any — that closed it out. A stage
    with no divisions was agreed without one ("on the nod")."""
    db = request.app.state.supabase
    result = (
        db.table("bill_stages")
        .select(
            "id, stage_name, house, sort_order, last_update, bill_stage_sittings(sitting_date), "
